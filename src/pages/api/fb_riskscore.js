...
// async function fetchUserData(username) {
  ...
  // let nextCursor = null;
  // const MAX_POSTS = 40;
  const MAX_POSTS = parseInt(process.env.MAX_POSTS || "15", 10);
  ...
  const commentFetches = allPosts.map(async (post) => {
    ...
    const cJson = await cRes.json();
    // const rawComments = Array.isArray(cJson.results) ? cJson.results : [];
     // only grab the first N comments to speed things up
    const rawComments = Array.isArray(cJson.results)
      ? cJson.results.slice(0, 5)
      : [];
    return simplifyData(rawComments, "comment");
  });
      ...
      Object.keys(finalRelationships).length > 0
        ...
        : {
            [username]: {
              username,
              relationship: "None",
              profileImage: mainUserProfileImage,
            },
          };
