...
#    "darksearchio": "http://darksearch.io",
  #  "notevil": "http://hss3uro2hsxfogfq.onion",  # Offline?
   ...
   # "tordex": "http://tordex7iie7z2wcg.onion",  # Offline?
    ...
    #"deeplink": "http://deeplinkdeatbml7.onion",  # Offline?v
...
def darksearchio(searchstr):
    ...
    if max_number_of_pages  != 0:
        max_nb_page = max_number_of_pages 
...
def scrape_onion(email):
    ...
    if threads_to_use and threads_to_use > 0:
        units = threads_to_use
    else:
    ...
    for res in results:
        if email in str(res) or email.split('@')[0] in str(res):
            finall_results.append(res)
    ...
    for s in stats_dict:
        ...
        total += n
    if len(finall_results) == 0:
        return "Nothing found in tor network"
    if len(finall_results) > 10:
        return finall_results[:10]
