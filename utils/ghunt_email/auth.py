...
import json
import base64
import utils.ghunt_email.listener as listener
from typing import *
from utils.ghunt_email.knowledge import services_baseurls
from utils.ghunt_email.utils import parse_oauth_flow_response
from utils.ghunt_email.base import GHuntCreds
from utils.ghunt_email.errors import *
from utils.ghunt_email.knowledge import get_domain_of_service, get_package_sig
from utils.ghunt_email.accounts import Accounts
from bs4 import BeautifulSoup as bs

def check_osid(as_client: httpx.Client, cookies: Dict[str, str], service: str) -> bool:
    """Checks the validity of given OSID."""
    domain = get_domain_of_service(service)
    wanted = ["authuser", "continue", "osidt", "ifkv"]
    req =  as_client.get(f"https://accounts.google.com/ServiceLogin?service={service}&osid=1&continue=https://{domain}/&followup=https://{domain}/&authuser=0",
...
def check_osids(as_client: httpx.Client, cookies: Dict[str, str], osids: Dict[str, str]) -> bool:
    """Checks the validity of given OSIDs."""
    tasks = [check_osid(as_client, cookies, service) for service in osids]
    return all(tasks)

def check_master_token(as_client: httpx.Client, master_token: str) -> str:
    """Checks the validity of the android master token."""
    try:
        android_oauth_app(as_client, master_token, "com.google.android.play.games", ["https://www.googleapis.com/auth/games.firstparty"])
    except GHuntAndroidAppOAuth2Error:
        return False
    return True

def android_master_auth(as_client: httpx.Client, oauth_token: str) -> Tuple[str, List[str], str, str]:
    """
...
def gen_osid(as_client: httpx.Client, cookies: Dict[str, str], generated_osids: dict[str, str], service: str) -> None:
    domain = get_domain_of_service(service)
    ...
    req = as_client.get(f"https://accounts.google.com/ServiceLogin", params=params, cookies=cookies, headers=config.headers)
    ...
    req = as_client.post(f"https://{domain}/accounts/SetOSID", cookies=cookies, data=params, headers=headers)
...
def gen_osids(as_client: httpx.Client, cookies: Dict[str, str], osids: List[str]) -> Dict[str, str]:
    """
...
def gen_cookies_and_osids(as_client: httpx.Client, ghunt_creds: GHuntCreds, osids: list[str]=[*services_baseurls.keys()]):
    accounts_api = Accounts(ghunt_creds)
    is_logged_in, uber_auth = accounts_api.OAuthLogin(as_client)
    if not is_logged_in:
    ...
    req = as_client.get("https://accounts.google.com/MergeSession", params=params)
    cookies = dict(req.cookies)
    ghunt_creds.cookies = cookies
    osids = gen_osids(as_client, cookies, osids)
    ghunt_creds.osids = osids

def check_cookies(as_client: httpx.Client, cookies: Dict[str, str]) -> bool:
    """Checks the validity of given cookies."""
    continue_url = "https://www.google.com/robots.txt"
    params = {"continue": continue_url}
    req =  as_client.get("https://accounts.google.com/CheckCookie", params=params, cookies=cookies)
...
def check_and_gen(as_client: httpx.Client, ghunt_creds: GHuntCreds):
    """Checks the validity of the cookies and generate new ones if needed."""
    if not  check_cookies(as_client, ghunt_creds.cookies):
        gen_cookies_and_osids(as_client, ghunt_creds)
        if not  check_cookies(as_client, ghunt_creds.cookies):
...
def load_and_auth(as_client: httpx.Client, help=True) -> GHuntCreds:
