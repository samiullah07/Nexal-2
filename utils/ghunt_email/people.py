...
from utils.ghunt_email.utils import is_default_profile_pic
from utils.ghunt_email.apis import Parser
...
class PersonPhoto(Parser):
    ...
    def _scrape(self, as_client: httpx.Client, photo_data: Dict[str, any], photo_type: str):
        if photo_type == "profile_photo":
            self.url = photo_data.get("url")

            self.isDefault, self.flathash = is_default_profile_pic(as_client, self.url)
...
class Person(Parser):
    ...
    def _scrape(self, as_client: httpx.Client, person_data: Dict[str, any]):
            ...
            for profile_data in person_data["readOnlyProfileInfo"]:
                ...
                if person_data.get("photo"):
                    for photo_data in person_data["photo"]:
                        ...
                        person_photo._scrape(as_client, photo_data, "profile_photo")
        ...
        if person_data.get("coverPhoto"):
            for cover_photo_data in person_data["coverPhoto"]:
                ...
                person_cover_photo._scrape(as_client, cover_photo_data, "cover_photo")
