...
from utils.ghunt_email.parsers import Calendar, CalendarEvents
from utils.ghunt_email.base import GHuntCreds
from utils.ghunt_email.utils import TMPrinter
from utils.ghunt_email.apis import CalendarHttp


def fetch_all(ghunt_creds: GHuntCreds, as_client: httpx.Client, email_address: str) -> Tuple[Boolean, Calendar, CalendarEvents]:
    calendar_api = CalendarHttp(ghunt_creds)
    found, calendar =  calendar_api.get_calendar(as_client, email_address)
    if not found:
        return False, None, None
    tmprinter = TMPrinter()
    _, events =  calendar_api.get_events(as_client, email_address, params_template="max_from_beginning")
    next_page_token = deepcopy(events.next_page_token)
    while next_page_token:
        tmprinter.out(f"[~] Dumped {len(events.items)} events...")
        _, new_events =  calendar_api.get_events(as_client, email_address, params_template="max_from_beginning", page_token=next_page_token)
...
def out(calendar: Calendar, events: CalendarEvents, email_address: str, display_name="", limit=5):
    ...
    calendar_output += f"Calendar ID : {calendar.id}\n"
    if calendar.summary != calendar.id:
        print(f"[+] Calendar Summary : {calendar.summary}")
        calendar_output += f"[+] Calendar Summary : {calendar.summary}\n"

    print(f"Calendar Timezone : {calendar.time_zone}\n")
    calendar_output += f"Calendar Timezone : {calendar.time_zone}\n"
    ...
    if target_events:
        ...
        calendar_output += f"[+] {len(events.items)} event{'s' if len(events.items) > 1 else ''} dumped ! Showing the last {len(target_events)} one{'s' if len(target_events) > 1 else ''}...\n"
        ...
        calendar_output += "[-] No events dumped."
    ### Names
